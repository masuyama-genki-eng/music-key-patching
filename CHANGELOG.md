...
## 2026-09-09 — 追加実験B（擾乱閾値の感度）を実施し、補足資料へ新節として追記

指示書の実行順 B が先頭だったため最初に実施。**生成はゼロ**（全条件に継続ごとの
行があり、推定調と擾乱値を持つので厳密に再採点できる）。
新規: `experiments/reanalysis/threshold_sensitivity.py`、
artifact `results/reanalysis/b_threshold/{threshold_sensitivity.json,
sr_by_threshold.csv}`＋台帳。

対象は指示書の要求を上回る範囲: 自前モデル5アーム × 2旋法、steering 3条件 ×
2旋法（install を並置）、公開モデル8セル（Bach 5 + pop 3）。閾値は
0.2–1.6 のグリッド＋保存済みパーセンタイル（p50/p75/p90=凍結/p99）＋限度なし。

**結果: 順位の反転は 0 件**（`ordering_never_flips: true`）。
- 自前: どの閾値でも置換 > 全対照（長調・短調）。凍結閾値での順位との Spearman は
  τ=0.2 を除く全点で 1.000（τ=0.2 では対照同士が入れ替わり 長調 0.900 / 短調 0.975、
  置換の首位は不動）。
- 限度なしの値が既存 artifact の `raw_tkr`（長調 0.410、短調 0.6173）と一致 →
  再採点コードの整合性チェックとして機能した。
- 音高位置のみは全閾値で一定（長調 0.035、短調 0.000）。音楽をほとんど変えないので
  予算が効かない → 位置の帰無結果は採点規則の産物ではない、と言える。
- steering: 変位一致に対しては全閾値・両旋法で据え付けが勝つ。**2倍変位の加算は
  向きが閾値に依存**（長調では全点で加算が上、短調では凍結閾値付近で交差し
  0.505 対 0.494、限度なしでは据え付けが 0.617 対 0.599 で逆転）→ 正直に記載。
- 公開モデル: 予算はほぼ効いていない（凍結と限度なしがほぼ同値。MMT のみ
  0.700 対 0.832 で明確に支払う）。

補足資料に §4「Does the budget decide the result?」を新設（表 S14、日本語訳の塊つき）。
冒頭の「どこを見ればよいか」表にも1行追加。14ページになった。
- 数値の追跡: `collect_paper_numbers.py` に b_threshold ブロックを追加し、
  937 個の閾値関連値を artifact から再計算して出力。`--check-tex` は
  **untraceable 0 件**を維持。
- 未解決として明記: per-event の上昇値が未保存のため p80/p85/p95 は算出不可
  （要・参照モデル再実行）。移調参照は擾乱値を持たないため再採点対象外。
- ラベル修正: 位置対照の節に `\label{sec:position}` が無かったので追加（参照先の明確化）。

## 2026-09-09 (2) — 既存データで埋まる2件を補足へ追記（短調の着地表 / 据え付け対加算の効果曲線）

いずれも**生成ゼロ**。指示書の D2 と E の一部に相当する。

**1. 短調の着地表（D2）** — `results/reanalysis/a2/landing.json` に短調の6セル×3条件が
既に計算済みだったが、補足の表 S9 は長調のみだった。表を2旋法に拡張し、旋法固有の
疑い（短調は相対長調へ流れて的中と採点されうるのでは）に答える段落を追加:
編集が相対長調に着地するのは 0.041、同主長調 0.045 で、**どちらも統制と同じ大きさ**
（変位一致で 0.043 / 0.047）→ 編集が生んだ移動ではない。長調の残りを支配していた
五度隣接（0.288）は短調では 0.037 しかない。

**2. 据え付け対加算の効果曲線（E）** — 新規 `experiments/reanalysis/steering_pareto.py`、
artifact `results/reanalysis/e_pareto/{pareto.json,pareto.csv}` ＋台帳。
探索段階が α ∈ {0.25,…,16} × 8層を掃いて per-row を残していたので、α ごとに
KS のみの SR・閾値つき SR・擾乱・音階内割合を読み戻した。変位は行に無いが
構成上 α·s̄(層) で厳密（`s_bar.json`）。**読み方は数値を見る前に固定**して記録。

結果は**層で分かれ、片側は我々に不利**なのでそのまま書いた:
- 第4層（据え付けの層）: 加算は掃いた**どの変位でも届かない**（最良 0.219 対 0.402）。
  押しを強めると悪化する。
- 第2層（加算自身が選んだ層）: 変位を揃えれば据え付けが勝つ（0.276 対 0.155）が、
  1.353倍で追いつき、2倍で追い越す（0.497）。
- **擾乱で見ると逆**: 第2層 α=2 で加算は成功率が高く擾乱中央値も低い（−0.059 対 0.162）。
  → 「据え付けが効率的」は**制御量（変位）あたりであって、損害量（擾乱）あたりではない**
  と限定して書き直した。第2層で据え付けが高くつく理由は幾何（変位20.3/活性長49.7=41%、
  第4層は 34.6/168.5=21%）。
- 加算に固定点が無いことが表に現れる: α=8 で擾乱中央値 24,015 nat、α=16 で 86,216 nat、
  一方で成功率は崩壊。据え付けは構成上標的を超えられないのでこれが起きない。

補足は15ページ。照合器に pareto ブロックを追加し `--check-tex` は untraceable 0 を維持。
369 tests passed。

## 2026-09-09 (3) — 実験A（ピッチクラス対照部分空間）: 事前登録 → stage 1 完了

**事前登録**: `docs/ADDITIONAL_EXPERIMENTS_FREEZE.md` を `results/reanalysis/a_pitchclass/`
が存在しない時点でコミット（c68ae44）。凍結内容は主設計の完全踏襲（層4、rank24の
probe V、予算0.613、最終テストのプロンプト、GEN_SEED 7）＋**結果の読み方を先に固定**
（主張を弱める2つの帰結も含め、うち1つは本文にも書くと明記）。指示書からの逸脱1件
（ランダム対照を各1個。計算量のため）も記録。

**回帰のバグを dry-run で発見・修正（報告値を出す前に）**: 最初の実装は切片なしで
X のみ中心化していた。ピッチクラスヒストグラムは平均が大きく（各クラスが 1/12 付近、
音階内はさらに上）、原点を通る当てはめは容量をその offset の再現に費い、**相関 0.867
なのに R² が負**という産物を出していた。切片を入れて修正:
- w=16: R² 0.235 → **0.654**（探索プロンプト）/ 0.199 → **0.631**（保持分割）
- w=512: R² −0.034 → **0.781** / −0.317 → **0.754**
分布シフトか否かを切り分けるための in-distribution 保持分割も診断として追加した
（λ の選択は凍結どおり探索プロンプトのみで行う）。

**stage 1 の結果**（`results/reanalysis/a_pitchclass/overlap.json`、50,400 訓練位置）:
- λ = 1e-3（探索プロンプト上の平均 R² 0.718 で最良）
- 実現ランク: V=24、V_pc12=11、V_pc24=**22**、V_res=**24**
- **重なり: V と V_pc24 は 0.0556、ランダム24次元の帰無 0.0470 ± 0.0027**
  → 帰無からわずか約3.2SD、絶対値では V のエネルギーの 5.6% のみが内側
- **主角22本すべてが 63.6°〜89.4°、30°未満は0本**
- V_res が満ランク24 = ピッチクラス部分空間を射影除去しても V は崩れない
→ 幾何的には「V はピッチクラス頻度の線形像ではない」。因果テストは stage 2 で判定。

**stage 2 の実装**: `experiments/reanalysis/pitchclass_edit.py`。ゲート（V_pc24 経由の
sham が無編集出力をトークン単位で再現）→ 6アーム × 12標的 × 100プロンプト × 2旋法。
install は再生成せず台帳済み parquet から対応付け。dry-run（5プロンプト×2標的）で
2件の実装ミスを修正: エディタは {層: editor} の辞書で返す必要があり、かつ
`generate_batch` は**引数の個数**でアリティを判定するため1引数のクロージャで渡す必要が
あった（tests/test_editor_arity.py が同じ規約を守らせている）。本番は約2.2時間。

## 2026-09-09 (4) — 本文に図2「層ごとの可読性 対 編集効果」を追加、§4.4 を簡潔化

新規 `experiments/figures/fig_read_use.py` → `paper/fig_read_use.pdf`（本文で使用）
および `paper/fig_read_use_margin.pdf`（指定どおりのマージン版、代替）。
8層分の値は artifact から読み、手入力はしていない。

**指定との相違を1件、理由つきで記録**: 著者指定は左軸を「統制補正 macro-F1 −
最強の音符数えベースライン」（マージン）としていたが、その定義では**乖離が消える**:
マージンは第1層でピークの17%、編集効果は15%で、両者がほぼ並走する。さらに第1層の
マージンの CI は [−0.0001, +0.0212] でゼロを含み（補足§2 も「未決着」と記載）、
指定の文案「probe margin is already positive at layer 1」は支えられない。
一方、生の macro-F1 なら第1層でピークの94%に対し編集効果は15%で乖離が明確であり、
これは現行本文が既に依拠している値（0.872）でもある。よって**本文の図は生の
macro-F1 版**を用い、マージン版も同時に出力して1行差し替えで切替可能にした
（図のキャプション直上の日本語コメントに手順を記載）。

**§4.4 の簡潔化**: 図が担う5つの数値（0.872 / 0.04 / 0.19–0.30 / 0.921 / 0.927）を
本文から削除し、トピック文→図の参照→主張の回収の順に再構成。著者指示により
散文からコロンとセミコロンを排除（既存の2箇所も書き換え）。1文1行を維持。

図の仕様: 1カラム幅（86mm）、左軸 probe macro-F1（0–1 の全範囲、軸切り詰めなし）、
右軸 edit gain、実線+白抜き丸 と 破線+塗り四角（白黒印刷でも判別可）、ゼロ線は薄灰、
serif フォント・labelsize 7.5–8 で既存図と整合、ベクター PDF。
ページ数は 6 → 6（増えていない。ただし**投稿前から 4+1 規定を超過**しており、
これは本タスク以前からの未解決事項）。表1・表2 の配置は不変。

## 2026-09-09 (5) — 図2の配色と枠線を参照図に合わせた（著者指示）

`experiments/figures/fig_read_use.py`:
- 線色を muted slate/brick から **青 (blue) / オレンジ (orange)** へ（参照図と同系）。
- 枠線を**全周（上下左右）の黒 0.8pt** に（従来は上を非表示・薄灰）。目盛も黒。
- ゼロ線は薄灰 #BBBBBB のまま。**線種（実線／破線）とマーカー（白抜き丸／塗り四角）の
  区別は維持**（前回指示の白黒印刷対応）。フォントは本文図と揃えた serif のまま。
両版（生F1版 fig_read_use.pdf、マージン版 fig_read_use_margin.pdf）に同じ変更を適用。
ページ数 6 のまま、375 tests green。

## 2026-09-09 (6) — 図2の微調整（著者指示3点）

1. **丸マーカーを廃止** — probe 線を白抜き丸から**塗り三角**へ。edit gain の四角と
   合わせて白黒印刷でも判別できる組み合わせを維持。
2. **枠線を薄く** — 純黒 0.8pt → **#4D4D4D 0.7pt**（目盛も同色 0.7pt）。ゼロ線も
   #BBBBBB → #CCCCCC。
3. **右軸ラベルを明示** — 「edit gain」→ **「edit gain (SR − baseline)」**。
   著者から「SR と edit gain は違うのか」という問いがあったため、図の中で差分である
   ことが分かるようにした（キャプションの定義文はそのまま）。
   数値関係の確認: 最終テストの SR は 0.3555（表1）でランダム同次元は 0.0391 なので
   その差は 0.3164。図2の右軸は**探索段階**の差で L4 が 0.303。段階が違うので値も
   違い、キャプションに「both from the search stage」と明記してある。
ページ数 6 のまま、375 tests green。

## 2026-09-09 (7) — 図2の青線に情報を持たせ、本文を図と整合させた

著者指示「青線もただの真っ直ぐな線ではなく工夫したい」への対応。装飾ではなく
**情報を足す**方向で工夫した:
- **区間ごとの線種**: 両端の層がどちらも最強の音符数えに勝っている区間は実線、
  そうでない区間は点線（第0–2層が点線）。マーカーも塗り分け（塗り三角=勝つ層、
  白抜き=勝たない層。第0層はマージンが負、第1層は CI がゼロを跨ぐ）。
- **帯**: 左は6モデルの範囲（`results/probing/*/probe_report.json`）、右は層ごとの
  プロンプト単位 BCa ブートストラップ95% CI（sweep parts から新規計算）。
  6モデルはほぼ一致するため左の帯は細いが、これは「曲線の形が特定の学習実行に
  固有ではない」という情報そのもの。

**本文を図と整合させた（重要）**: 図が第1層を白抜き＝「音符数えを超えていない」と
示すため、旧文の「第0層だけを比較の外に置く」では図と矛盾する。§4.4 を
- 点線部分の意味を述べる1文
- 「第1層までに飽和するのは**生の** probe 精度であり、プロービング研究が通常報告する
  のはその量で、編集が効く層を突き止められないのもその量である」という1文
に書き換えた。これは以前の報告で指摘した論点（マージンで測ると乖離が消える）を
図と本文の両方で明示したことになり、鋭い査読者の指摘を先回りできる。
コロン・セミコロンは散文・キャプションとも不使用を維持。ページ数 6、375 tests green。

## 2026-09-09 (8) — 図2: 青の帯を削除、オレンジ線を実線に（著者指示）

- **青の6モデル帯を削除**。6モデルがほぼ一致するため帯が細く、情報量に対して
  ノイズになっていた。青線の情報（区間ごとの線種＋マーカーの塗り分け）は維持。
- **オレンジ線を破線から実線へ**。白黒印刷での判別は、三角と四角のマーカーの違い、
  および青線の点線区間が担う。
...
## 2026-09-09 (10) — 実験A完了: 「V はピッチクラス頻度の線形像ではない」を確定

事前登録 docs/ADDITIONAL_EXPERIMENTS_FREEZE.md（c68ae44、artifact ゼロの時点で
コミット）に従い実行。両旋法とも**読み方1（主張が強まる）**に該当した。
台帳: 2026-09-09 の EXP A stage 1 / stage 2 × 2旋法。

**stage 1（幾何）**: 層4の活性 → 直近ピッチクラスヒストグラムのリッジ回帰（訓練
50,400位置、λ=1e-3 は探索プロンプトで選択）。当てはまりは良い（探索プロンプト上
R² w=16 0.654 / w=512 0.781、保持分割 0.631 / 0.754）にもかかわらず、
V との重なりは 0.056（ランダム24次元の帰無 0.047±0.003）、主角22本すべて
63.55°–89.40° で30°未満は0本、V から射影除去しても V は満ランク24。

**stage 2（因果）**: 同じ μ_κ* をピッチクラス部分空間経由で書き込むと
長調 0.0273 / 短調 0.0027 にとどまり、**同次元・同変位のランダム部分空間
（0.0245 / 0.0036）と区別できず 0/12**。11次元の全履歴版も同じ（0.0300 / 0.0027）。
逆に V からピッチクラス部分空間を除去した残差で書き込むと **長調 0.3600 /
短調 0.4782** に達し、据え付け（0.3555 / 0.4945）との対応差は
長調 −0.0045 [−0.0371, +0.0300]、短調 +0.0164 [−0.0218, +0.0518] で**ゼロを含む**。
自前対照に対しては 12/12 で有意。音階内割合も残差 0.935 / 0.957 対
ピッチクラス側 0.551 / 0.728（無編集水準）。

**書き足し**: 補足に §13「Is the subspace a linear image of pitch-class frequency?」
（表 S16、日本語訳の塊つき）を新設し、冒頭の「どこを見ればよいか」表にも1行追加。
本文 §4.4 にも3文追加（この反論は査読者が必ず突くため、補足に埋めず本文で回収）。
照合器に実験Aの51値を追加し `--check-tex` は untraceable 0 を維持。
補足15ページ、本文6ページ（増えていない）、375 tests green。

## 2026-09-10 — C1 完了（公開3モデルの生成前シフト）を補足に追記

AMENDMENT 1 の C1。3チェックポイントすべてで、編集は次音の対数比を無編集より
大きく動かし、REMI と MMT では符号ごと反転させた（AMT −0.0239→−0.0096、
REMI −0.1048→+0.5171、MMT −0.6895→+0.2051）。ランダム部分空間との**対応のある
比較**は AMT +0.0119（r 0.505、p=3.4e-4）、REMI +0.5955（r 0.995）、
MMT +0.8197（r 1.000、いずれも p≤1.1e-11）。

**途中で直した2つの実装欠陥**（いずれも報告値を出す前）:
- ピッチクラスへの畳み込みが AMT の平坦語彙前提でインライン実装されており、
  REMI（`model.net` はテンソルを返す）でクラッシュ、MMT の複合方式では表現不可。
  アダプタ契約に任意メソッド `next_pitch_class_mass` を追加して各方式に閉じ込めた。
  **リファクタ後に AMT の公表値が完全再現**することを確認してから他2つを実行。
- MMT の `CODE_PITCH_MAP` は非ピッチコードに None を持つため TypeError。None を除外。

**測定の追加（読み方の変更ではない）**: 凍結した読み方は「編集で動き対照では動かない」
という**対比**だが、既存スクリプトは各条件を無編集に対してのみ検定していた。REMI と
MMT では対照もわずかに有意に動く（+0.026、+0.075）ため、対比そのものを測る
対応のある edit 対 k1 検定を追加した。既存の条件別検定と判定文は不変。差は対照の
20〜30倍なので、対照の小さな動きは編集の効果を説明しない。この経緯も補足に明記。

補足 §24 に「All three checkpoints」の小節と表を追記（日本語訳の塊つき）。
照合器に C1 の値を追加、`--check-tex` untraceable 0、387 tests green、補足15ページ。

## 2026-09-10 (2) — D3 を短調でも実行（CPUのみ）し、既存の主張を1つ狭めた

AMENDMENT 1 §8 が「未実行」として挙げていた短調の推定器差し替えを実施。継続トークン列
（results/rescore/continuations_R-Aug_s0_minor_L4.json）が保存済みなので**生成不要・
CPU のみ**で、GPU の E2 と競合せずに実行できた。artifact: results/reanalysis/a9_minor/。

結果: 短調でも**5推定器すべてで編集が対照を上回る**（率 0.4755–0.6236、対照
0.0200–0.0264）。

**既存の主張を1つ狭めた（重要）**: 補足は「事前登録した推定器が5つの中で最も低い率を
与える（＝最も保守的）」と書いていたが、これは**長調でのみ真**。短調では Viterbi の
0.4755 が Krumhansl–Kessler の 0.4945 より 0.019 低く、事前登録した推定器は2番目に
低い。§17 と冒頭の索引表の両方を旋法つきの表現に修正した。

副産物: 短調の優位は推定器により +0.051（Bellman–Budge）から +0.182（Aarden–Essen）
まで動く。優位自体はどの推定器でも残るが、その大きさは測定の性質であり、§18 の
「短調の優位は測定由来」という主張を裏づける。
補足16ページ、`--check-tex` untraceable 0、387 tests green。

## 2026-09-10 (3) — C3 完了（公開モデルの層をまたぐ read–use gap）: 本文§4.4の弱点を埋めた

AMENDMENT 1 の C3。**生成ゼロ** — 層別プローブ（mwild_probe.json）と層スキャン
（stage1_layer_scan.json）はどちらも既存 artifact で、同じ軸に載せただけ。
新規 `experiments/reanalysis/public_layer_gap.py` →
`results/reanalysis/c3_layer_gap/gap_music-small-800k.json`＋
`results/figures/supp/supp_public_layers.pdf`＋台帳。

**これは本文の弱点を埋める**。図2（我々のモデル）では、音符数えに対するマージンが
編集効果とほぼ並走するため、乖離は「生の精度」についてのものだった（この限界は
図2の追加時に報告済み）。AMT-small では**マージン自体が乖離する**:
- 編集効果の頂点は第8層（+0.287）、プローブとマージンの頂点は第10層
- **第0層: マージン +0.124（第7層の +0.127 と同等）に対し編集効果 −0.004（床）**
- **第11層: マージン +0.188（ほぼ最大）に対し編集効果 +0.029**
つまり公開モデルでは、統制した可読性の指標が編集の効く層を特定できていない。

注意も明記: 編集側は探索段階の層スキャンで1点あたり240継続（20プロンプト×12標的）。
図に Wilson 95%区間を描き、隣接層の細かい上下は読み過ぎないよう本文に書いた。

書き足し: 補足に §25「The read--use gap across a public model's layers」（図 S9b）、
本文 §4.5 に2文（コロン・セミコロン不使用）。照合器に C3 の値を追加、
`--check-tex` untraceable 0、389 tests green、補足16ページ、本文6ページ。

## 2026-09-10 (4) — E2 完了: 据え付けは段階的、しかし我々の目標値は最適でない

AMENDMENT 1 の E2。新モード `replace_scaled`（s=1 で replace とビット一致、s=0 で恒等。
tests/test_replace_scaled.py の4件で固定）で s ∈ {0.5,0.75,1.25,1.5} を最終テストの
両旋法で実行（s=1 は凍結行を再利用、生成せず）。artifact: results/reanalysis/e2_scaled/。

**事前登録の読み方3件のうち2件に同時該当、うち1件は我々に不利**:
- 読み方1（成り立つ）: s=1 まで単調（長調 0.099→0.222→0.356、短調 0.064→0.256→0.494）。
  据え付けは段階的な操作であり、特定の大きさでのみ働く閾値現象ではない。
- 読み方3（**我々に不利**）: s>1 でさらに上がる。長調 s=1.25 で 0.425、s=1.5 で 0.457、
  短調で 0.618 と 0.662。対応差の CI はすべてゼロを除外。**クラス平均そのものを書く
  という我々の選択は最適ではない**。
- 代償の有無: 長調 s=1.25 は擾乱中央値 0.093（s=1 は 0.104）でガード通過率 0.855
  （0.840）と、**追加の代償なしに成功率が上がる**。s=1.5 では擾乱 0.151・通過率 0.812。
  短調 s=1.25 はほぼ同等（0.183 対 0.180、0.785 対 0.792）。
- なぜクラス平均が最適でないかの説明は持っていないと明記（「平均はその調を最も強く
  表す点ではない」という読みは未検証）。**凍結設計は変えず、報告値は s=1 のまま**。

補足 §7 に「The install, weakened and strengthened」＋表 S17 を追記（日本語訳の塊つき）、
冒頭の索引表にも1行追加、`\label{sec:steering}` を新設。補足17ページ、
`--check-tex` untraceable 0、389 tests green。

## 2026-09-10 (5) — C2 完了: 位置の非対称は3つのトークン化で成り立つ

AMENDMENT 1 の C2 と、その追加条件 AMENDMENT 2。

**AMT-small（L8, Bach, 60プロンプト×12標的）— 読み方1**:
全位置 0.3653（k1 0.0639、11/12）に対し、音高位置のみ 0.0597（**0/12**、対照と同水準）、
時刻・音長位置のみ 0.3514（**11/12**、全位置の 96%）。位置シェアは 33.2% と 66.3%。
我々のモデル（音高 0.035 / 小節+音長 0.233）と同じ方向で、より極端。

**REMI（L5）— 最初は読み方3、追加条件で説明された**:
音高のみ 0.0694・タイミングのみ 0.0611（ともに 0/12、制限なし 0.4208）。
2マスクの被覆は **75.9%** で、抜けていたのは instrument トークン
（`probe_offset = −1` = 音高が選ばれる直前 = REMI のプローブが読む位置）。
**AMENDMENT 2 として「null を見た後に選んだ追加条件であり事前登録テストではない」と
明記して凍結**した上で実行 → **instrument のみで 0.4306（制限なしの 102%）、11/12 有意**、
音階内割合 0.881（他2条件は 0.58 台＝無編集水準）。3分類の被覆合計 99.6%。

**まとめ**: 3つのトークン化に共通するのは「音高トークンそのものでは効かない」ことで、
効く位置はいずれも次の音高が決まる直前の構造トークン（我々=小節・音長、AMT=時刻・音長、
REMI=楽器）。本文 §4.4 の「音高確率を押すだけの介入なら逆になる」を独立3方式で裏づけた。

書き足し: 補足 §26（表 S18）＋索引表1行、本文 §4.5 に1文、RESULTS.md を更新。
実装は `--position-kinds` で種類を可変に（既定は従来の2つ）、REMI に `instrument` を追加。
補足18ページ、本文6ページ、`--check-tex` untraceable 0、389 tests green。

## 2026-09-10 (6) — 追加実験の結果に合わせて総括節と Limitations を更新

追加実験の結果はすべて各節に書き足してあったが、**それらを要約する2箇所が古いまま**
だったので直した。文書内の整合が崩れている状態を放置しないため。

**補足の総括節「Where this leaves the claims」**:
- 「弱める発見は4件」→ **5件**。第5に E2 の結果（据え付けの標的が最適でない。
  s=1.25 で長調 0.4245・短調 0.6182、長調では擾乱中央値も低くガード通過率も高い。
  理由の説明は持たないと明記、凍結設計は不変で報告値は s=1）。
- 「生き延びない反論」の側に**4件追加**（いずれも追加実験由来で、この一覧で最も強い）:
  ピッチクラス対照（R² 0.78 でよく当てはまるのに V と重ならず因果的に無力）、
  予算の閾値（0.2 から限度なしまで順位不変）、位置の非対称（2公開方式で音高トークンは
  無効、効くのは音高が選ばれる直前の構造トークン）、公開モデルでの統制指標による
  read–use gap（両端の層でマージンが高いのに編集は無効）。
- 推定器の一文を旋法つきに訂正（長調では最低、短調では2番目）。

**本文 Limitations**: 「追加の因果統制は自前モデルのみ」は C1（3チェックポイント）と
C2（2チェックポイント）の実施で**偽になっていた**ので書き直した。残る限界を正確に:
MMT の位置対照は複合方式のため実施せず（「音高位置」が位置を名指さない）、層スイープと
ピッチクラス対照は自前モデル外では未実施。

補足18ページ、本文6ページ、`--check-tex` untraceable 0、389 tests green。

## 2026-09-10 (7) — ピアノロールを「背景ほぼ白・音符は濃く」で描き直せるようにした

著者指示。図1（paper/fig1.pdf）は**リポジトリのスクリプトが生成していない外部合成図**で、
右側にピアノロールが貼られている。そのピアノロール自体は
`experiments/figures/pianoroll_figure.py` の出力であり、既存7配色は地が濃く音符が
明るいものが多かった。

追加（既存の挙動・配色は1バイトも変更していない）:
- 配色 `clear`（地 #ffffff、プロンプト #4a6785、継続 **#13223a**）と
  `clearmoss`（地 #ffffff、#5d8a6e、継続 **#12331f**）。
- `--prompt-alpha`（既定 0.78 = 従来）。1.0 にするとプロンプトの音符も継続と同じ濃さ。
- `--from-dump`: 保存済み `pianoroll_tokens.json` から**再生成せず**描き直す。
  音符が台帳済みレンダと同一であることが保証され、GPU も不要（F1 の実行と競合しない）。
- `--palettes`: 描く配色を選べる（既定は全部＝従来）。

生成物: `results/figures/pianoroll_light/pianoroll_F_to_E_{clear,clearmoss}.{pdf,png}`
（`--bg-strength 0.12 --prompt-alpha 1.0`）。音符・小節帯・示された調はすべて既存の
台帳済みトークン列そのままで、**数値や音楽内容は一切変わっていない**。
