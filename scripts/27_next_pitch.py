"""S3 — the next-pitch causal test: zero-feedback logit-level evidence.

The sustained-edit result admits a feedback reading: the edit changes early
notes, the notes announce the new key, the model follows its own evidence
(experiment G2 showed exactly this pathway carries persistence). This test
removes the pathway entirely: no token is ever sampled. Each held-out prompt is
extended by the teacher-forced structural prefix [BAR, POS_1] (fixed, key-neutral)
and the edit is applied from the BAR position; we read the next-token
DISTRIBUTION at the last position, restricted to PITCH tokens.

Frozen metrics (docs/CONFIRMATORY_FREEZE.md @ 0d621e4): P_target = probability
mass on pitches diatonic to the injected key; Delta_key = log P_target -
log P_source; primary statistic D = Delta_key(cond) - Delta_key(clean), edit vs
K1, one-sided Wilcoxon per target on non-identity cells, Holm across 12.

Artifacts: results/confirmatory/<model>/next_pitch.json + parquet + ledger.
"""
...
confirm = __import__("26_confirmatory")
...
def main() -> None:
    ...
    for pi, p in enumerate(prompts):
        ...
        for tgt in MAJOR_TARGETS:
            ...
            for cond, pr in (("clean", clean), ("edit", e), ("k1", k)):
                ...
                rows.append({"prompt_idx": pi, "target_key": tgt,
                             "src_key": p.src_key, "cond": cond,
                             "p_target": pt, "p_source": ps,
                             "delta_key": float(np.log(max(pt, 1e-12))
                                                - np.log(max(ps, 1e-12)))})
        if (pi + 1) % 20 == 0:
    ...
    piv = df.pivot_table(index=["prompt_idx", "target_key", "src_key", "identity"],
                         columns="cond", values="delta_key").reset_index()
    ...
    for tgt in MAJOR_TARGETS:
        ...
        t = wilcoxon_rank_biserial(d["D_edit"].values, d["D_k1"].values,
                                   alternative="greater")
        pvals.append(t["p"])
        recs.append({"target": tgt, "n": len(d),
                     "mean_D_edit": float(d["D_edit"].mean()),
                     "mean_D_k1": float(d["D_k1"].mean()), **t})
    for r, p_adj in zip(recs, holm_correct(pvals)):
        r["p_holm"] = float(p_adj)
    pv = df.pivot_table(index=["prompt_idx", "target_key", "identity"],
                        columns="cond", values="p_target").reset_index()
    nip = pv[~pv["identity"]]
