...
def main() -> None:
    ...
    out = Path(args.out)
    if out.exists():
        raise SystemExit(f"{out} already exists — the frozen guard must not be "
    ...
    snapshot(out, cfg)
    log.info("FROZEN delta_ppl = %.4f (P%.0f of %d events)", delta, args.percentile,
             len(arr))
    if not args.no_ledger:
        append_entry(stage="P4 guard freeze (delta_PPL)", config=cfg, seeds=None,
