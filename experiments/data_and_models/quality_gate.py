...
from src.tokenizer.vocab import VOCAB, IVOCAB
...
def val_reference(val_path: Path, n: int) -> dict:
    ...
    for seq in tbl.column("token_ids").to_pylist():
        ...
        if v is not None:
            ikrs.append(v)
    a = np.array(ikrs)
    return {"n": len(a), "ikr_mean": float(a.mean()), "ikr_std": float(a.std(ddof=1)),
...
@torch.no_grad()
def generate_unconditional(model: TonalGPT, n: int, gen_cfg: dict, device: str) -> list[list[int]]:
    ...
    while len(out) < n:
        ...
        seq = model.generate(ids, n_new=int(gen_cfg["max_new_tokens"]) - 1,
...
def eval_model(model_dir: Path, gen_cfg: dict, n_gen: int, device: str) -> dict:
    ...
    return {
        ...
        "gen": {"n": len(pieces), "ikr_mean": float(a.mean()),
...
def main() -> None:
    ...
    log.info("frozen thresholds: top1 >= %.4f (%.1fx const-predictor %.4f); "
    ...
    model_dirs = sorted(d for d in Path(args.models).iterdir()
    ...
    for d in model_dirs:
        ...
        log.info("%s: top1 %.4f (>=%.4f: %s)  gen IKR %.4f (>=%.4f: %s)",
                 d.name, m["val_top1"], top1_threshold, m["gate"]["top1_pass"],
                 m["gen"]["ikr_mean"], ikr_threshold, m["gate"]["ikr_pass"])

    report["all_pass"] = all(m["gate"]["pass"] for m in report["models"].values())
    out_path = outdir / "quality_gate.json"
    out_path.write_text(json.dumps(report, indent=2))
    snapshot(out_path, {"gen": gen_cfg, "n_gen": args.n_gen, "n_val_ref": args.n_val_ref})
    log.info("wrote %s (all_pass=%s)", out_path, report["all_pass"])

    if not args.no_ledger:
        append_entry(
            stage="P2 quality gate (SPEC §2.1)",
            config={"gen": gen_cfg, "n_gen": args.n_gen, "n_val_ref": args.n_val_ref,
