...
def main() -> None:
    ...
    ap.add_argument(
        "--only", default=None, help="train only the named run, e.g. R-Aug_s0"
    )
    ...
    def add(name: str, regime: str, seed: int, train_pq: str, val_pq: str) -> None:
        ...
        runs.append(
            TrainRun(
                regime=regime,
                seed=seed,
                train_parquet=str(data / train_pq),
                val_parquet=str(data / val_pq),
                out_dir=str(outdir / name),
                model=cfg["model"],
                lr=float(o["lr"]),
                betas=tuple(o["betas"]),
