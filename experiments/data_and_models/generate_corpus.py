...
from src.datagen.generator import GenConfig, generate_piece
...
def _split_cfg(cfg: dict, split: str, n: int) -> dict:
    ...
    fields = {f.name for f in dataclasses.fields(GenConfig)} - {"seed"}
    return {k: cfg[k] for k in fields} | {
...
def split_stats(rows: list[dict]) -> dict:
    ...
    return {
        ...
        "initial_key_hist": dict(
            sorted(Counter(r["initial_key"] for r in rows).items())
        ),
        "per_token_key_occupancy": dict(sorted(label_occupancy.items())),
        "n_key_changes_hist": dict(
            sorted(Counter(r["n_key_changes"] for r in rows).items())
        ),
        "mod_type_counts": dict(Counter(t for r in rows for t in r["mod_types"])),
        "mod_fifths_hist": dict(
            sorted(Counter(d for r in rows for d in r["mod_fifths"]).items())
        ),
        "pct_pieces_with_modulation": float(
            np.mean([r["n_key_changes"] > 0 for r in rows])
        ),
    }
...
def main() -> None:
    ...
    ap.add_argument(
        "--no-verify", action="store_true", help="skip byte-identity regeneration gate"
    )
    ap.add_argument(
        "--no-ledger",
        action="store_true",
    ...
    for split, n in splits:
        ...
        if not args.no_verify:
            ...
            if sha256_file(tmp) != file_hash:
                tmp.unlink()
                raise RuntimeError(
                    f"P1 GATE FAILED: {split} regeneration not byte-identical"
                )
            tmp.unlink()
