"""Compare linear probes against extended note-history baselines (Sec. 4.1).

Reproduces the stored probe and piece-label-control scores before evaluating
longer windows, decayed histories, and concatenated short/long histograms.
"""
...
from src.probing.extract import (
    PITCH_ID_HI,
    PITCH_ID_LO,
    extract,
    load_corpus,
...
def probe_preds_from_weights(
    acts: np.ndarray, W: np.ndarray, b: np.ndarray, mask: np.ndarray
) -> np.ndarray:
...
def main() -> None:
    ...
    data = extract(
        model, seqs, labels, per_seq, min_pos, old_windows + EXT_WINDOWS, seed, device
    )
    del model
    ...
    out: dict = {
        "model": name,
        "experiment": "D (C3 window extension)",
        "gates": {},
        "c3": {},
        "probe_from_stored_weights": {},
    }
    ...
    for w in old_windows:
        c3_results[f"lr_W{w}"] = C.c3_pc_hist_lr(
            data[f"pc_hist_W{w}"], y, masks, seed, device
        )
        got = c3_results[f"lr_W{w}"]["report"]["macro_f1_24"]
        want = report["c3"][f"lr_W{w}"]["macro_f1_24"]
        out["gates"][f"G1_lr_W{w}"] = {
            "got": got,
            "ledgered": want,
    ...
    if not all(g["pass"] for k, g in out["gates"].items() if k.startswith("G1")):
        raise SystemExit(
            "G1 FAILED: re-extraction does not reproduce the ledgered "
    ...
    for li in range(L):
        yp = probe_preds_from_weights(
            data["acts"][li], pw[f"layer_{li}"], pw[f"bias_{li}"], test_mask
        )
        probe_pred[li] = yp
    ...
    if not g2_ok:
        raise SystemExit(
            "G2 FAILED: stored probe weights do not reproduce the "
    ...
    for li in range(L):
        r = train_probe(
            data["acts"][li].astype(np.float32), y_c1b, masks, base_cfg, device=device
        )
    ...
    if not g3_ok:
        raise SystemExit(
            "G3 FAILED: C1b retrain does not reproduce the ledgered "
    ...
    for lam in DECAY_HALFLIVES:
        h = np.stack(
            [decayed_hist(seqs[si], int(t), lam) for si, t in zip(seq_idx, pos)]
        )
    ...
    log.info(
        "best C3 overall: %s F1=%.4f (pre-registered best: %s %.4f)",
        best_name,
        best["report"]["macro_f1_24"],
        report["verdict_DR_H1"]["best_c3"],
        0.7902,
    )
    ...
    verdict = {
        "model": name,
        "rule": "DR-H1 (experiment D: strengthened C3)",
        "best_c3": best_name,
        "layers": [],
    }
    for li in range(L):
        ...
        ci = bca_ci(
            np.arange(conf_p.shape[0]),
            stat,
            n_boot=int(rc["probe"]["bootstrap"]["n"]),
            seed=seed,
        )
        ...
        log.info(
            "L%d: corrected diff %.4f CI [%.4f, %.4f] excl0=%s",
            li,
            ci["stat"],
            ci["ci_lo"],
            ci["ci_hi"],
    ...
    if not args.no_ledger:
        append_entry(
            ...
            seeds=[seed],
            artifacts=[
                str((pdir / f).resolve().relative_to(REPO))
                for f in ("c3_window_ext.json", "verdict_DR-H1_extD.json")
            ],
