"""Linear key probes, piece-label controls, and note-history baselines.

Writes probe weights in raw activation coordinates and class means for
activation replacement; all splits of probe examples are by piece.
"""
    ...
    for li in range(acts.shape[0]):
        ...
        for kind in kinds:
            r = train_probe(
                ...
                y,
                masks,
                ProbeConfig(**{**cfg.__dict__, "kind": kind}),
                device=device,
            )
        ...
        log.info(
            "  layer %d: "
            + " ".join(
                f"{k} F1={out[k][li]['report']['macro_f1_24']:.4f}" for k in kinds
            ),
            li,
        )
    return out
...
def main() -> None:
    ...
    data = extract(
        model, seqs, labels, args.per_seq, args.min_pos, windows, seed, device
    )
    del model
    ...
    # ---------------- linear probes and piece-label control
    log.info("training linear probes")
    main_probes = probe_all_layers(
        data["acts"], y, masks, base_cfg, device, kinds=("linear",)
    )
    log.info("training piece-label controls (linear)")
    ...
    for li in range(data["acts"].shape[0]):
        ...
        conf_b, _ = per_sequence_confusions(
            y_test, c1b["linear"][li]["y_pred_test"], sidx_test
        )
        stat = seq_f1_stat({"probe": conf_p, "c1b": conf_b, "c3": conf_c3})
        ci = bca_ci(
            np.arange(conf_p.shape[0]),
            stat,
            n_boot=int(pcfg["bootstrap"]["n"]),
            seed=seed,
        )
        ...
        log.info(
            "layer %d: corrected diff %.4f CI [%.4f, %.4f] excl0=%s",
            li,
            ci["stat"],
            ci["ci_lo"],
            ci["ci_hi"],
