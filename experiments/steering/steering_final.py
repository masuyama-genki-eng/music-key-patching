...
def main() -> None:
    ...
    ap.add_argument("--conditions", default="B,C")
    ...
    if not picks_path.exists():
        raise SystemExit(
            "frozen_picks.json missing — the search stage must choose "
    ...
    s_bar = {
        int(k): v["s_bar"]
        for k, v in json.loads((outdir / "s_bar.json").read_text())["per_layer"].items()
    }
    ...
    inst = pd.read_parquet(
        REPO / "results/confirmatory" / inst_name / "parts/confirmatory_L4.parquet"
    )
    ...
    for cond in args.conditions.split(","):
        ...
        if cache.exists():
            ...
            log.info(
                "%s: guarded SR %.3f (install %.3f)",
                cond,
                results[cond]["sr_guarded"],
                results[cond]["install_sr"],
            )
            continue
        log.info(
            "%s: layer %d alpha %s — clean twins then %d targets",
            cond,
            li,
            alpha,
            len(targets),
        ...
        for tgt in targets:
            ...
            rows, _ = SW.rows_for_condition(
                ...
                mref,
                device,
                clean_ppl(model, mref, prompts, cfg, device, args.batch_size),
            )
    ...
    if not args.no_ledger:
        note = "; ".join(
            f"{c}: SR {r['sr_guarded']:.3f} vs install "
            f"{r['install_sr']:.3f}, {r['n_sig_holm']}/12 sig"
            for c, r in results.items()
        )
        append_entry(
            stage=f"Steering FINAL test ({name}{suffix})",
            config=vars(args),
            seeds=[confirm.GEN_SEED],
            artifacts=[str(vpath.relative_to(REPO))],
            note=note,
        )
...
def clean_ppl(model, mref, prompts, cfg, device, batch):
    ...
    if key not in _CLEAN_CACHE:
        ...
        _, ppl = SW.rows_for_condition(
            {"cond": "clean"}, prompts, conts, mref, device, None
        )
...
def summarize(df: pd.DataFrame, inst: pd.DataFrame, targets, cond) -> dict:
    ...
    for tgt in targets:
        ...
        pairs = [
            (bool(inst_idx.get((r.prompt_idx, tgt), False)), bool(r.succ))
    ...
    per_prompt = (
        (d.groupby("prompt_idx").succ.mean() - inst.groupby("prompt_idx").succ.mean())
        .dropna()
        .values
    )
    ci = bca_ci(np.arange(len(per_prompt)), lambda idx: float(np.mean(per_prompt[idx])))
