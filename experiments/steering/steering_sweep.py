"""Steering search stage: conditions B / C / D of docs/STEERING_FREEZE.md §1.

Everything runs on the SAME machinery as the ledgered install sweep — the same
search prompts, per-batch rng discipline, M-REF scoring, and the frozen quality
budget — so the only degrees of freedom are the ones the plan names: the mode
(add_matched / add_fixed / add_contrast), the layer, and for C/D the single
dimensionless alpha shared by every position, prompt and target.

Invocations are cumulative like the layer scan: each (condition, layer, alpha)
cell writes its own parquet under results/steering/<model>/parts/, and
search_summary.json keeps the union, so grid extensions (the frozen asymmetric
stopping rules, §4) add cells without re-running old ones.
"""
...
def main() -> None:
    ...
    ap.add_argument("--alphas", default="0.25,0.5,1,2,4,8,16",
                    help="C/D only; ignored for B (whose displacement is delta(t))")
    ...
    if not (reg.exists() and json.loads(reg.read_text()).get("passed")):
        raise SystemExit("regression gate has not passed on this stack — run "
    ...
    clean_conts = json.loads((REPO / "results/sweep" / name /
                              "clean_conts.json").read_text())
    _, clean_ppl = SW.rows_for_condition({"cond": "clean"}, prompts, clean_conts,
                                         mref, device, None)

    mode = MODES[args.condition]
    layers = [int(x) for x in args.layers.split(",")]
    alphas = ([None] if args.condition == "B"
    ...
    def cell(li: int, alpha: float | None) -> dict:
        ...
        for tgt in MAJOR_TARGETS:
            ...
            conts = SW.generate_batch(model, prompts, ed_fn, {**gen_cfg, "layer": li},
                                      device, batch, seed=args.seed)
            rows, _ = SW.rows_for_condition(
                {"cond": args.condition, "mode": mode, "layer": li,
                 "alpha": alpha, "target_key": tgt},
        ...
        return {"condition": args.condition, "mode": mode, "layer": li,
                "alpha": alpha,
    ...
    summary = (json.loads(summary_path.read_text())
               if summary_path.exists() else {"cells": []})
    done = {(c["condition"], c["layer"], c["alpha"]) for c in summary["cells"]}
    for li in layers:
        for a in alphas:
            if (args.condition, li, a) in done:
                log.info("skip %s L%d a=%s (already in the summary)",
                         args.condition, li, a)
                continue
            res = cell(li, a)
            summary["cells"].append(res)
            summary_path.write_text(json.dumps(summary, indent=2))
            log.info("%s L%d a=%-5s  SR %.3f (raw %.3f)  guard %3.0f%%  "
                     "IKR tgt %.3f src %.3f", args.condition, li, str(res["alpha"]),
                     res["sr_guarded"], res["sr_unguarded"],
                     100 * res["guard_rate"], res["ikr_target"], res["ikr_src"])
    ...
    if not args.no_ledger:
        ...
        append_entry(stage=f"Steering search {args.condition} ({name})",
