"""Compute layer-specific normalization for fixed-length addition.

Average ||P mu_target - P h|| over search-prompt positions and all major targets.
"""
...
def main() -> None:
    ...
    if not args.no_ledger:
        append_entry(
            stage=f"Steering s_bar ({name})",
            config=vars(args),
            seeds=[],
            artifacts=[f"results/steering/{name}/s_bar.json"],
            note="per-layer s_bar: "
            + ", ".join(f"L{li} {v['s_bar']:.2f}" for li, v in per_layer.items()),
        )
