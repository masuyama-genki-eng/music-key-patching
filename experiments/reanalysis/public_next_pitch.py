...
def main() -> None:
    ...
    prompts = build_prompts(
        adapter, chorales, args.n_prompts, max_tokens=adapter.context_length(model) - 8
    )
    ...
    def next_pc(prompt_ids: list[int], basis, tgt: int | None):
        ...
        ed = None
        if tgt is not None:
            ed = HookSubspaceEditor(basis, mu[tgt], mode="replace", from_position=0)
    ...
    tab = (
        df.groupby("cond")[["target_mass", "source_mass", "log_ratio"]].mean().round(4)
    )
    log.info(
        "next-pitch scale mass at the predict-pitch position:\n%s", tab.to_string()
    )
        ...
        recs.append(
            {
                ...
                "effect_r": round(r["r"], 3),
                "p_raw": float(f"{r['p']:.3g}"),
            }
        )
        pv.append(r["p"])
    ...
    log.info(
        "installed vs prompt key, log mass ratio:\n%s",
    ...
    rek = wilcoxon_rank_biserial(
        jj.a.to_numpy(), jj.b.to_numpy(), alternative="greater"
    )
    edit_vs_k1 = {
        "n_prompts": int(len(jj)),
    ...
    log.info(
        "edit vs random subspace, paired: difference %+.4f (r=%.3f, p=%.3g)",
        edit_vs_k1["difference"],
        edit_vs_k1["effect_r"],
        edit_vs_k1["p"],
    )
    ...
    if moved and not ctrl_moved:
        verdict = (
            "the edit shifts the public model's FIRST decision toward the "
        ...
        verdict = (
            "both the edit and a random subspace shift the first decision; the "
            "shift is not specific to the key subspace here"
        )
    else:
        verdict = (
            "the first decision does not move, although generation under the "
    ...
    (outdir / f"next_pitch_{short}_L{args.layer}.json").write_text(
        json.dumps(
            {
                "model": args.model,
                "layer": args.layer,
    ...
    if not args.no_ledger:
        append_entry(
            stage=f"Re-analysis 11: pre-generation logits, {short} L{args.layer}",
            config=vars(args),
            seeds=[args.seed],
            artifacts=[
                str(
                    (outdir / f"next_pitch_{short}_L{args.layer}.json").relative_to(
                        REPO
                    )
                )
            ],
