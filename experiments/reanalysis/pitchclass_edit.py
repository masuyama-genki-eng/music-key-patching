"""Evaluate pitch-class and residualized key subspaces (Sec. 4.3).

Both subspaces have their own rank- and displacement-matched random control.
Uses the same held-out prompts and scoring rules as the main final test.
"""
...
SUBSPACES = ["pc24", "res"]
CTRL_SEED_OFFSET = {"pc24": 1, "res": 3}  # frozen in the document
...
def main() -> None:
    ...
    ap.add_argument("--arms", default="pc24,res")
    ...
    targets = (
        list(range(args.n_targets))
        if args.mode == "major"
        else [t + 12 for t in range(args.n_targets)]
    )
    log.info(
        "%s: %d prompts (rows %d-%d), %d targets%s",
        args.mode,
        len(prompts),
        rows_used[0],
        rows_used[-1],
    ...
    sham = gen(lambda plen: {LAYER: SW.make_editor(V24, None, device, mode="sham")})
    bad = sum(c != s for c, s in zip(clean, sham))
    if bad:
        raise SystemExit(f"GATE FAILED: sham through V_pc24 differs on {bad} prompts")
    log.info(
        "gate PASSED: sham through V_pc24 reproduces clean output (%d/%d)",
        len(clean),
        len(clean),
    )
    ...
    for name in args.arms.split(","):
        V = sub[{"pc24": "V_pc24", "res": "V_res"}[name]]
    ...
    if args.dry_run:
        log.info(
            "DRY RUN summary:\n%s",
            df[~df.identity].groupby("cond").succ.mean().round(4).to_string(),
        )
        return

    # ---------------- statistics, exactly the primary rules
    inst_p = REPO / (
        "results/confirmatory/R-Aug_s0/parts/confirmatory_L4.parquet"
        if args.mode == "major"
        else "results/confirmatory/R-Aug_s0_minor/parts/confirmatory_L4.parquet"
    )
    ...
    for name in args.arms.split(","):
        ...
        for tgt in targets:
            ...
            recs.append(
                {
                    "target": int(tgt),
                    "sr": round(float(ee["succ"].mean()), 4),
                    "sr_ctrl": round(float(kk["succ"].mean()), 4),
                    **t,
                }
            )
            pv.append(t["p"])
        ...
        diff_by_prompt = (
            m.groupby("prompt_idx")
            .apply(
                lambda g: g["succ_i"].mean() - g["succ_a"].mean(), include_groups=False
            )
            .values
        )
        ci = bca_ci(
            np.arange(len(diff_by_prompt)),
    ...
    for cond, v in verdict["arms"].items():
        log.info(
            "%-11s SR %.4f (unguarded %.4f, guard %.2f, dim %d)",
            cond,
            v["sr"],
            v["sr_unguarded"],
            v["guard_pass"],
    ...
    if not args.no_ledger:
        append_entry(
            ...
            note="; ".join(f"{c} SR={v['sr']:.4f}" for c, v in verdict["arms"].items())
