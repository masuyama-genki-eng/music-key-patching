"""Is the key subspace just a linear image of pitch-class frequency? (experiment A)

Design and readings are frozen in docs/ADDITIONAL_EXPERIMENTS_FREEZE.md, which
was committed before this script produced anything.

Stage 1 (this file, --stage fit): regress layer-4 activations onto the
L1-normalized pitch-class histogram of the last w tokens, for w = 16 and
w = 512, on TRAINING positions only. lambda is chosen on search-stage prompt
positions. Three subspaces come out of it -- the 12-row full-history one, the
24-row stack that matches V's rank, and the part of V that survives projecting
the stack out -- together with how much they overlap V and how much key a probe
can read from the 24-row one.

Stage 2 (--stage edit): write mu_kappa* through each of those subspaces instead
of through V, on the final-test prompts, and score exactly as the primary run
does. The install arm is not regenerated: its rows are read from the ledgered
confirmatory parquet.

Artifacts: results/reanalysis/a_pitchclass/{subspaces.npz, subspaces.json,
overlap.json, records.jsonl, verdict.json} + ledger.
"""
...
import confirmatory_test as confirm                  # frozen prompts + GEN_SEED
...
def hist_features(ids_list: list[list[int]], acts: np.ndarray,
                  meta: dict) -> dict[int, np.ndarray]:
...
def stage_fit(args) -> None:
    ...
    seqs, labels = load_corpus(str(REPO / "results/data_syn/train.parquet"),
                               n_seqs=args.n_fit_pieces)
    fit = extract(model, seqs, labels, per_seq=24, min_pos=8, windows=WINDOWS,
                  seed=0, device=device, compute_ambiguity=False)
    ...
    sel = extract(model, sseqs, slabels, per_seq=24, min_pos=8, windows=WINDOWS,
                  seed=0, device=device, compute_ambiguity=False)
    ...
    for lam in LAMBDAS:
        ...
        scores[lam] = {
            "per_window_r2_search_prompts": {str(w): round(v, 4)
                                             for w, v in per_w.items()},
            "per_window_r2_heldout_fit_positions": {str(w): round(v, 4)
                                                    for w, v in per_w_id.items()},
    ...
    V = v_probe(np.load(Path(args.probing_dir) / "probe_weights.npz")
                [f"layer_{LAYER}"], rank=RANK)

    V_pc12, r12 = orthonormal(Wraw[512])
    ...
    null = []
    for _ in range(100):
        G = rng.standard_normal((V.shape[0], RANK))
        Q, _ = np.linalg.qr(G)
        null.append(overlap(V, Q))
    angles = np.degrees(np.arccos(np.clip(
        np.linalg.svd(V.T @ V_pc24, compute_uv=False), -1, 1)))

    ov = {
        "freeze": "docs/ADDITIONAL_EXPERIMENTS_FREEZE.md",
        "lambda_selection": {"candidates": {str(k): v for k, v in scores.items()},
                             "chosen": best,
        ...
        "ranks": {"V": int(V.shape[1]), "V_pc12": r12, "V_pc24": r24,
                  "V_res": rres},
        "overlap_V_with_V_pc24": round(overlap(V, V_pc24), 4),
        "overlap_V_with_V_pc12": round(overlap(V, V_pc12), 4),
        "overlap_V_with_random_r24": {
            "mean": round(float(np.mean(null)), 4),
            "sd": round(float(np.std(null)), 4),
            "n_draws": 100, "seed": 20260909},
        "principal_angles_deg_V_vs_V_pc24": [round(float(a), 2) for a in angles],
        "n_angles_below_30deg": int((angles < 30).sum()),
        "energy_of_V_inside_V_pc24": round(
            float((np.linalg.norm(V_pc24.T @ V, "fro") ** 2) / RANK), 4),
    }
    OUT.mkdir(parents=True, exist_ok=True)
    np.savez(OUT / "subspaces.npz", V=V, V_pc12=V_pc12, V_pc24=V_pc24,
             V_res=V_res, W_pc16=Wraw[16], W_pc512=Wraw[512])
    (OUT / "overlap.json").write_text(json.dumps(ov, indent=2))
    for f in ("subspaces.npz", "overlap.json"):
        snapshot(OUT / f, vars(args), seeds=[0])
    log.info("overlap V vs V_pc24 = %.4f (random null %.4f +/- %.4f); ranks %s",
             ov["overlap_V_with_V_pc24"], ov["overlap_V_with_random_r24"]["mean"],
             ov["overlap_V_with_random_r24"]["sd"], ov["ranks"])
    if not args.no_ledger:
        append_entry(stage="EXP A stage 1: pitch-class regression subspaces "
                           "(freeze ADDITIONAL_EXPERIMENTS_FREEZE.md)",
                     config=vars(args) | {"lambda": best}, seeds=[0],
                     artifacts=[f"results/reanalysis/a_pitchclass/{f}" for f in
                                ("subspaces.npz", "overlap.json")],
                     note=f"lambda={best:g}; ranks {ov['ranks']}; overlap "
                          f"{ov['overlap_V_with_V_pc24']} vs random null "
                          f"{ov['overlap_V_with_random_r24']['mean']}")


def main() -> None:
    ap = argparse.ArgumentParser()
    ap.add_argument("--stage", choices=["fit", "edit"], required=True)
    ...
    if args.stage == "fit":
        ...
        raise SystemExit("stage 'edit' lives in pitchclass_edit.py")
