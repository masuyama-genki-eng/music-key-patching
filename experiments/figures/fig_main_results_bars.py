...
def draw(out_base: Path, width_mm: float, height_mm: float) -> None:
    ...
    panels = [
        (axes[0], sr_repl, sr_ctrl, "SR (%)",
         "(a) Continuation", (0.0, 58.0), "{:.1f}"),
        (axes[1], delta_repl, delta_ctrl, r"$\delta D$",
         "(b) Before sampling", (0.0, 0.84), "{:.3f}"),
    ]
    ...
    for ax, repl, ctrl, ylabel, title, ylim, fmt in panels:
        ...
        ax.set_ylim(*ylim)
        ax.set_ylabel(ylabel, fontsize=8.0, color=INK, labelpad=2.0)
        ...
        for bars in (b1, b2):
            for bar in bars:
                ...
                ax.text(bar.get_x() + bar.get_width() / 2, v + label_pad,
                        ...
                        fontsize=6.1, color=INK, rotation=0)
    ...
    fig.legend(handles, labels, loc="upper center", bbox_to_anchor=(0.52, 1.01),
               ...
               columnspacing=1.6)
    fig.subplots_adjust(left=0.105, right=0.985, bottom=0.20, top=0.78,
                        wspace=0.36)
