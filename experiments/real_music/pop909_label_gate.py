...
def main() -> None:
    ...
    for p in pieces:
        ...
        # KS estimate anyway.
        for lab, grp in groupby(
            zip(p["events"], p["event_key_labels"]), key=lambda x: x[1]
        ):
    ...
    if n == 0:
        raise SystemExit(
            "LABEL GATE FAILED: no segment reached the minimum note "
    ...
    corpus_stats = {
        ...
        "n_multi_key_pieces": sum(
            len({k for _, k in p["key_changes"]}) > 1 for p in pieces
        ),
    ...
    out = {
        "corpus": "POP909-CL (POP909_processed)",
        "n_pieces": stats["n_pieces"],
        "corpus_stats": corpus_stats,
        "n_segments_scored": n,
        "n_segments_skipped_lt16_notes": n_skipped,
        "breakdown": dict(cats),
    ...
    if not args.no_ledger:
        append_entry(
            stage="POP909-CL key-label gate",
            config=vars(args),
            seeds=[],
    ...
    if not passed:
        raise SystemExit(
            "LABEL GATE FAILED: the human key labels disagree with the "
