...
primary analysis (S2) and reported as a sanity check. The pitch and bar/duration arms share the same prompts and clean twins.
...
ARMS = {
    "edit": None,
    "pitch": MASKS["pitch"],
    "bar_dur": MASKS["bar_dur"],
}


def select_prompts_holdout(
    test_parquet: str, n: int, prompt_bars: int = 8, mode: str = "major"
):
...
def main() -> None:
    ...
    ap.add_argument(
        ...
        type=int,
        default=4,
        help="layer selected by the search; paper uses 4",
    )
    ap.add_argument(
        "--arms",
        default="edit,pitch,bar_dur",
        help="comma-separated subset of edit,pitch,bar_dur",
    )
    ...
    if bad:
        raise SystemExit(
            f"K2 GATE FAILED on holdout prompts: {bad}/{len(prompts)} differ"
        )
    log.info("K2 gate PASSED (%d/%d bit-identical)", len(prompts), len(prompts))
    ...
    for cond, mask_fn in {**arms, "k1": None, "k1_norm": None}.items():
        ...
        def make_editor_fn(t, b=basis, r=ref):
            ...
            def ed_fn(plen):
                return SubspaceEditor(
                    torch.from_numpy(b).float().to(device),
    ...
    verdict = {
        "freeze": run_cfg["freeze"],
        "model": name,
        "layer": args.layer,
        "prompt_rows": run_cfg["prompt_rows"],
        "conditions": {},
    }
    for cond in arms:
        ...
        if cond == "edit":
            ...
            verdict["edit_vs_k1norm"] = {
                "per_target": recs_n,
                "n_sig_holm": sum(1 for r in recs_n if r["p_holm"] < 0.05),
        ...
        verdict["conditions"][cond] = {
            ...
            "identity_sanity": {
                "n": int(len(ident)),
                "guarded_tkr": float(ident["succ"].mean()) if len(ident) else None,
            },
        }
        log.info(
            "%-8s guarded %.4f vs K1 %.4f | %d/12 Holm-significant | guard %.3f",
            cond,
            verdict["conditions"][cond]["pooled_guarded_tkr"],
            verdict["conditions"][cond]["pooled_k1"],
            nsig,
    ...
    if not args.no_ledger:
        append_entry(
            ...
            note="; ".join(
                f"{c}: guarded={verdict['conditions'][c]['pooled_guarded_tkr']:.3f} "
