"""Checks for replacement, norm-matched addition, and fixed-length addition."""
...
@pytest.mark.parametrize("mode", ["add_fixed"])
def test_alpha_zero_reproduces_the_unedited_state_exactly(mode):
    V, mus, h = _fixture()
    kw = {}
    ed = SubspaceEditor(
        V, mus[7], mode=mode, alpha=0.0, s_bar=34.63, from_position=FROM, **kw
    )
...
@pytest.mark.parametrize("mode", ["add_matched", "add_fixed"])
...
def test_replace_removes_the_old_component_and_adding_keeps_it():
    ...
    assert (_proj(V, steer)[:, FROM:] - want).norm() > (
        _proj(V, inst)[:, FROM:] - want
    ).norm(), "steering landed at least as close to the target component as install"
...
def test_generation_leaves_the_prompt_tokens_untouched():
    ...
    with torch.no_grad():
        ...
        edited = model.generate(
            ids, n_new=6, editors={MLAYER: ed}, rng=g2, edit_from=plen
        )
