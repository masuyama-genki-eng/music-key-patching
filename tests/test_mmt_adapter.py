...
EVENTS = [(0.0, 0.5, 60), (0.5, 0.5, 64), (1.0, 0.5, 67), (1.5, 0.5, 72),
          (2.0, 1.0, 71), (3.0, 0.5, 69), (3.5, 0.5, 65), (4.0, 1.0, 62)]
...
def test_editor_semantics_on_the_real_residual_stream(setup):
    ...
    inst(h); steer(h)
    rel = ((inst.last_delta_norm - steer.last_delta_norm).abs()
           / inst.last_delta_norm.clamp_min(1e-8)).max()
    assert rel < 1e-5
...
def test_encoding_gate_passes_on_a_synthetic_diatonic_piece(setup):
    ...
    for s in range(4):
        ...
        for k in range(48):
            ...
            t += 0.5
        pieces.append({"events": ev, "event_key_labels": [0] * len(ev),
                       "tempo_us": 500_000, "name": f"syn{s}"})
