...
def test_all_true_mask_reproduces_the_unmasked_edit():
    ...
    plain = HookSubspaceEditor(V.clone(), mu.clone(), from_position=2)(None, None,
                                                                      h.clone())
    masked = HookSubspaceEditor(V.clone(), mu.clone(), from_position=2,
...
def test_masked_positions_are_left_alone():
    ...
    out = HookSubspaceEditor(V, mu, from_position=0, token_mask=m)(None, None,
                                                                  h.clone())
