...
from src.publicmodels.pop909 import (
    EXCLUDED,
    bar_of_tick,
    key_index,
    load_pop909,
...
def test_split_is_deterministic_disjoint_and_piece_level(corpus):
    ...
    names = [p["name"] for p in pieces]
    cfg = yaml.safe_load(
        (Path(__file__).resolve().parents[1] / "configs/pop909.yaml").read_text()
    )["split"]
...
def test_zero_duration_notes_are_counted_not_vanished(tmp_path):
    ...
    track = bytes(
        [
            ...
            0x80,
            60,
            0,  # off at the same tick
            0,
            0x80,
            61,
            0,  # orphan off: nothing open on 61
            0,
            0xFF,
            0x2F,
            0,
