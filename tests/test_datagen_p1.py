...
from src.datagen.generator import GenConfig, generate_piece, Key
    ...
    for seed in range(50):
        ...
        for i in range(1, len(tokens)):
            if labels[i] != labels[i - 1]:
                assert tokens[i] == "BAR", f"seed={seed} pos={i}: label change at {tokens[i]}"
    ...
    for seed in range(400):
        ...
        for ev in events:
            if ev.get("modulation") != "pivot":
                continue
            chord = next(e for e in events
                         if e["bar"] == ev["bar"] and e["notes"] and e["pos"] == 1)
            pcs = {n % 12 for n in chord["notes"][:4]}   # SATB only; melody may be new-key
...
def test_modulation_markers_match_label_changes():
    ...
    for seed in range(100):
        ...
        changes = [(labels[i - 1], labels[i]) for i in range(1, len(labels))
                   if labels[i] != labels[i - 1]]
        assert marks == changes, f"seed={seed}: markers {marks} vs label changes {changes}"
