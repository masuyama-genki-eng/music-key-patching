...
IMPLICIT = {"__file__", "__name__", "__doc__", "__class__", "__module__",
            "__qualname__", "__spec__", "__package__", "__builtins__"}
...
@pytest.mark.parametrize("path", SCRIPTS + PACKAGES,
...
def test_nothing_is_read_after_del(path: Path):
    ...
    assert not offenders, (
        f"{path.relative_to(REPO)} reads freed names: {offenders}")
