...
- 2 並行の実測: 1 目標あたり 112–131 s（単独 53 s）。**並行してもスループットは増えない**（GPU 計算が 1 ジョブで飽和）。以後は逐次キューで GPU を空けずに回す。総 GPU 時間 ≈ 23 h はそのまま。

## 8. 進捗（2026-09-18 22:00）
- **Phase 1 完了**: T0（完全再現）、T2、T4、D1 再生成（3 モデル + δD）、T1（6 モデル × 2 モード）、T3 必須分（pc24 × 3）。残差変種 × 3 は低優先で実行中（採用は 9/20 まで）。
- **Phase 2 完了（著者判断待ちを除く）**: T5、T6 本文（4 + 1 ページ維持、`--check-tex` 0）、T6 補足（20 ページ）。Abstract / Conclusion の提案文は `paper_edits.md` に 3 案ずつ、未挿入。
- **Phase 3**: `results/PROVENANCE.md`、`paper_edits.md` 作成済み。最終ビルド確認は著者の文選択後に再実行。

- 2026-09-18 22:59 **QUEUE END**（FAIL 0）。残差変種 3 本も完了: REMI+ 0.567（V 0.644 を概ね保持）、MMT 0.453（0.694）、**AMT-12L 0.106（0.506）** — AMT では pc 部分と残差のどちらも単独では効かない。補足 §14 と本文 4.5 に反映済み。
