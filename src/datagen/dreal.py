...
def parse_chorale(path: str | Path) -> dict | None:
    ...
    for onset in sorted(by_onset):
        ...
        if cur_measure != mno:
            cur_measure = mno
            tokens.append("BAR"); labels.append(key); onsets.append(onset)
        tokens.append(f"POS_{pos}"); labels.append(key); onsets.append(onset)
        for midi, dur in sorted(by_onset[onset]):
            tokens.append(f"PITCH_{midi}"); labels.append(key); onsets.append(onset)
            tokens.append(f"DUR_{dur}"); labels.append(key); onsets.append(onset)
    tokens.append("EOS"); labels.append(key); onsets.append(onsets[-1])
    if not all(tk in VOCAB for tk in tokens):
        return None
    return {"tokens": tokens, "key_labels": labels, "key": key,
            "n_bars": len(bounds), "meter": meter, "onsets": onsets,
            "bars": dict(bounds), "beat_16ths": 16 // (meter[1] if meter else 4)}
...
def parse_romantext_keys(path: str | Path) -> list[tuple[int, float, int]] | None:
    ...
    for line in Path(path).read_text(encoding="utf-8", errors="replace").splitlines():
        ...
        for beat_s, key_s in BEAT_KEY_RE.findall(rest):
            ...
            pc = PC[letter.lower()] + (1 if acc == "#" else -1 if acc in ("b", "-") else 0)
...
def local_key_labels(chorale: dict, changes: list[tuple[int, float, int]]) -> list[int] | None:
...
def load_corpus_local(kern_dir: str | Path, analysis_dir: str | Path,
                      max_len: int = 512) -> tuple[list[dict], dict]:
    ...
    """
    analyses = {p.parent.name.lstrip("0") or "0": p
                for p in sorted(Path(analysis_dir).glob("*/analysis.txt"))}
    out: list[dict] = []
    stats = {"no_analysis": 0, "bwv_mismatch": 0, "key_mismatch": 0,
             "unparsable_score": 0, "no_key_changes": 0, "matched": 0}
