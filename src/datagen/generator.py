...
from src.tokenizer.vocab import POS_RES, VOCAB
...
def voice_chord(
    pcs: list[int], prev: list[int] | None, rng: np.random.Generator
) -> list[int]:
...
def generate_piece(cfg: GenConfig):
    ...
    if rng.random() < cfg.p_modulate:
        ...
        bars = sorted(
            rng.choice(np.arange(4, n_bars - 2), size=n_mod, replace=False).tolist()
        )
        for b in bars:
    ...
    for bar in range(n_bars):
        ...
        if seq_pending is not None:
            ...
            if mtype == "pivot":
                ...
                half = (
                    interval // 2 if interval % 2 == 0 and interval != 0 else interval
                )
