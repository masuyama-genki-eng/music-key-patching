...
STATUS: NOT executed in the planning container (no torch). Claude Code must run
tests/test_model_smoke.py in P0 before any training. Written conservatively against
standard torch APIs.
"""
...
class TonalGPT(nn.Module):
    ...
    def __init__(self, vocab_size: int, d: int = 512, n_layers: int = 8, n_heads: int = 8,
                 ctx: int = 512, dropout: float = 0.1):
        super().__init__()
        self.ctx = ctx
    ...
    def forward(self, ids: torch.Tensor, capture: bool = False, editors: dict | None = None):
        B, T = ids.shape
