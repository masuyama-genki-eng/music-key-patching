...
def evaluate(model: TonalGPT, val: torch.Tensor, batch_size: int, device: str) -> dict:
    ...
    return {"val_loss": nll, "val_ppl": math.exp(nll),
            "val_top1": tot_correct / tot_tokens, "val_tokens": tot_tokens}
...
def train_one(run: TrainRun) -> dict:
    ...
    if final_path.exists():
        ...
        if meta["config_hash"] == cfg_hash:
            log.info("%s: final checkpoint exists with matching config — skipping", out.name)
            return json.loads((out / "metrics.json").read_text())
        raise RuntimeError(f"{out}: exists with different config hash — refusing to overwrite")

    device = "cuda" if torch.cuda.is_available() else "cpu"
    seed_everything(run.seed, deterministic_torch=False)  # cudnn determinism too slow; run-level
    ...
    log.info("%s: train %s, val %s, device=%s", out.name, tuple(train_ids.shape),
             tuple(val_ids.shape), device)

    model = TonalGPT(vocab_size=len(VOCAB), **run.model).to(device)
    opt = torch.optim.AdamW(model.parameters(), lr=run.lr, betas=tuple(run.betas),
    ...
    while step < run.max_steps:
        ...
        if step % 100 == 0:
            ...
            log.info("step %d/%d loss %.4f lr %.2e (%.1f it/s)",
                     step, run.max_steps, loss.item(), lr_at(step - 1), rate)
        if step % run.eval_every == 0 or step == run.max_steps:
            ev = evaluate(model, val_ids, run.batch_size, device)
            curve.append({"step": step} | ev)
            log.info("eval @%d: loss %.4f ppl %.3f top1 %.4f",
                     step, ev["val_loss"], ev["val_ppl"], ev["val_top1"])
        if step % run.ckpt_every == 0 or step == run.max_steps:
            torch.save({"model": model.state_dict(), "opt": opt.state_dict(),
                        "step": step, "curve": curve, "config_hash": cfg_hash,
                        "cpu_rng": torch.get_rng_state()}, latest)

    torch.save({"model": model.state_dict(), "step": step, "config_hash": cfg_hash,
                "config": cfg}, final_path)
    snapshot(final_path, cfg, seeds=[run.seed])
    metrics = {"regime": run.regime, "seed": run.seed, "steps": step,
