...
def read_midi(path: str | Path) -> dict:
    ...
    if div & 0x8000:
        raise ValueError(
            f"{path}: SMPTE time division — this reader assumes "
    ...
    for _ in range(ntrk):
        ...
        while j < end:
            ...
            if st & 0x80:
                ...
                if running is None:
                    raise ValueError(
                        f"{path}: running status after a meta/sysex "
    ...
    return {
        "div": div,
        "tempo_us": tempo_us or DEFAULT_TEMPO_US,
        "n_tempo_events": n_tempo,
        "key_sigs": key_sigs,
        "time_sigs": time_sigs,
        "notes": notes,
        "n_unclosed": n_unclosed,
        "n_zero_duration": n_zero_dur,
        "n_orphan_offs": n_orphan_off,
    }
...
def load_pop909(
    root: str | Path, min_labeled_events: int = 32
    ...
    pieces, stats = (
        [],
        {
            ...
            "too_short": [],
            "n_files": len(files),
            "dropped_unlabeled_notes": 0,
            "dropped_zero_duration_notes": 0,
            "orphan_note_offs": 0,
        },
    )
    for f in files:
        ...
        pieces.append(
            {
                "name": name,
                "events": events,
                "event_key_labels": labels,
                "key": labels[0],
                "key_changes": ks,
                "div": m["div"],
                "tempo_us": m["tempo_us"],
                "time_sigs": m["time_sigs"],
...
def split_pieces(
    names: list[str], seed: int, frac: tuple[float, float, float]
) -> dict[str, list[str]]:
    ...
    n2 = int(len(order) * (frac[0] + frac[1]))
    return {
        "train": sorted(order[:n1]),
        "search": sorted(order[n1:n2]),
        "final": sorted(order[n2:]),
    }


def load_pop909_part(
    root: str | Path,
    part: str,
    seed: int,
    frac: tuple[float, float, float],
    min_labeled_events: int = 32,
) -> tuple[list[dict], dict]:
