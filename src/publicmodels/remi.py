...
class RemiAdapter(PublicModelAdapter):
    ...
    def check_vocab(self, model) -> None:
        ...
        if got != len(self.ev2c):
            raise RuntimeError(
                f"vocab mismatch: checkpoint {got} vs encoding "
    ...
    def encode_events(
        ...
        if self.seconds_per_beat is None:
            raise RuntimeError(
                "encode_events before set_piece_context: REMI needs "
        ...
        for onset_s, dur_s, pitch in events:
            ...
            if beat >= self._max_beat:
                ...
                continue
            notes.append((beat, position, pitch, max(1, round(dur_s / spb * res))))
    ...
    def encoding_is_sane(
        ...
        for ch in chorales[:n]:
            ...
            events = [
                e for e in events if e[0] / self.seconds_per_beat < self._max_beat
            ]
            if len(events) < 32:
                continue
