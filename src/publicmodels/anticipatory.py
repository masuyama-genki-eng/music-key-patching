...
def encode_events(
    events: list[tuple[float, float, int]], instrument: int = DEFAULT_INSTRUMENT
    ...
    for onset, dur, pitch in events:
        ...
        if t < 0:
            raise ValueError(
                f"negative onset {onset}s — a front-trim would desync "
        ...
        if not (0 <= pitch < MAX_PITCH):
            raise ValueError(
                f"MIDI pitch {pitch} out of range — dropping it "
    ...
    if (
        n_trimmed
        and note_positions
        and events[len(note_positions) - 1][0] > events[-1 - n_trimmed][0]
    ):
...
def continuation_events(ids: list[int]) -> list[tuple[float, float, int]]:
    ...
    while i + 2 < len(ids):
        ...
        if (
            TIME_OFFSET <= t < DUR_OFFSET
            and DUR_OFFSET <= d < NOTE_OFFSET
...
def check_vocab(model_vocab_size: int) -> None:
    ...
    if model_vocab_size != VOCAB_SIZE:
        log.info(
            "checkpoint vocab %d vs layout %d (+%d padding rows, unused)",
            model_vocab_size,
            VOCAB_SIZE,
...
def encoding_is_sane(model, chorales: list[dict], device: str, n: int = 12) -> dict:
    ...
    for ch in chorales[:n]:
        ...
        rng.shuffle(ev)
        ev = sorted(
            [(events[i][0], e[1], e[2]) for i, e in enumerate(ev)], key=lambda x: x[0]
        )  # keep the time grid, scramble pitches
...
class AnticipatoryAdapter(PublicModelAdapter):
    ...
    def _generate_step(
        ...
        token-identity-verified against the saved pre-refactor outputs."""
        logits = model(window).logits[:, -1][0]
