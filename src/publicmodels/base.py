...
class PublicModelAdapter(ABC):
    ...
    @staticmethod
    def nucleus_draw(
        logits: "torch.Tensor", temperature: float, top_p: float, rng: "torch.Generator"
    ) -> int:
                        ...
                        if getattr(editor, "mask_kind", None) is not None:
                            editor.token_mask = torch.as_tensor(
                                self.token_type_mask(
                                    window[0].tolist(), editor.mask_kind
                                )
                            )
    ...
    @abstractmethod
    def _generate_step(
        self, model, window, temperature: float, top_p: float, rng
    ) -> "torch.Tensor | None":
