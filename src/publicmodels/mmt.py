...
from src.publicmodels.corpus import chorale_to_events, events_of, presentation_tempo_us
...
SOS, SON, EOS = (
    R.TYPE_CODE_MAP["start-of-song"],
    R.TYPE_CODE_MAP["start-of-notes"],
...
class MMTAdapter(PublicModelAdapter):
    ...
    def load(self, checkpoint: str, device: str):
        ...
        model = MusicXTransformer(
            dim=args["dim"],
            encoding=copy.deepcopy(R.get_encoding()),
            depth=args["layers"],
            heads=args["heads"],
        ...
        state = torch.load(
            root / "checkpoints/best_model.pt", map_location=device, weights_only=False
        )
    ...
    def check_vocab(self, model) -> None:
        ...
        if got != want:
            raise RuntimeError(
                f"per-field vocab mismatch: checkpoint {got} vs "
    ...
    def residual_streams(self, model, ids: torch.Tensor) -> list[torch.Tensor]:
        ...
        handles = [
            self.block(model, li).register_forward_hook(
    ...
    def encode_events(
        ...
        if self.seconds_per_beat is None:
            raise RuntimeError(
                "encode_events before set_piece_context: MMT needs "
        ...
        if len(codes) and int(codes[-1][DIM["type"]]) == EOS:
            codes = codes[:-1]
        note_positions = [
            i for i, row in enumerate(codes) if row[DIM["type"]] == NOTE_TYPE
        ]
        ...
        if len(note_positions) != len(notes):
            raise RuntimeError(
                f"{len(notes) - len(note_positions)} notes fell "
    ...
    def _begin_generation(self, prompt_ids) -> None:
        ...
        self._cur_beat = (
            int(prompt_ids[0, note_rows, DIM["beat"]].max()) - 1
            if note_rows.any()
            else 0
        )
    ...
    def _generate_step(
        self, model, window, temperature: float, top_p: float, rng: torch.Generator
    ):
    ...
    def generate(
        self, model, prompt_ids, n_new, layer, editor, temperature, top_p, rng
    ):
        assert prompt_ids.shape[0] == 1, "the sweep generates one prompt at a time"
        self._stop_after = False
        return super().generate(
            model, prompt_ids, n_new, layer, editor, temperature, top_p, rng
        )

    # ---------------------------------------------------------- sanity
    @torch.no_grad()
    def encoding_is_sane(
        self, model, chorales: list[dict], device: str, n: int = 12
    ) -> dict:
        ...
        for ch in chorales[:n]:
            ...
            for i in npos:
                ...
                bad[i][DIM["pitch"]] = min(
                    n_pitch - 1, max(1, bad[i][DIM["pitch"]] + step)
                )
            shift.append(nll([tuple(r) for r in bad]))
            ev = list(events)
            rng.shuffle(ev)
            ev = sorted(
                [(events[i][0], e[1], e[2]) for i, e in enumerate(ev)],
                key=lambda t: t[0],
            )  # keep the grid, scramble pitches
        ...
        return {
            "nll_correct": r,
            "nll_pitch_shifted": s,
            "nll_time_scrambled": u,
            "sane": bool(r < s and r < u),
            "n": len(real),
        }
