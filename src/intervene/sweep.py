...
def select_prompts(
    test_parquet: str, n: int, prompt_bars: int = 8, major_only: bool = True
) -> list[Prompt]:
    """Pieces whose key is stable through the first `prompt_bars` bars."""
    tbl = pq.read_table(test_parquet, columns=["token_ids", "key_labels"])
    out = []
    for ids, lab in zip(
        tbl.column("token_ids").to_pylist(), tbl.column("key_labels").to_pylist()
    ):
...
# ------------------------------------------------------------------ editors
def make_editor(
    V: np.ndarray, mu_target: np.ndarray | None, device: str, mode: str = "replace"
) -> SubspaceEditor:
    Vt = torch.from_numpy(V).float().to(device)
    mt = (
        torch.from_numpy(mu_target).float().to(device)
