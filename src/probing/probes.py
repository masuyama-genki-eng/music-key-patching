"""Linear key probes with piece-level splits and training-only standardization."""
...
def macro_f1_from_conf(m: np.ndarray) -> float:
    ...
    f1 = np.divide(
        2 * prec * rec, prec + rec, out=np.zeros_like(tp), where=(prec + rec) > 0
    )
...
def _build(kind: str, d: int, k: int = 24) -> nn.Module:
    if kind == "linear":
        return nn.Linear(d, k)
    raise ValueError(f"Only linear probes are supported, got {kind!r}")
...
def train_probe(
    ...
    model = _build(cfg.kind, X.shape[1]).to(device)
    ...
    out = {
        "report": metric_report(yt["test"].numpy(), y_pred),
