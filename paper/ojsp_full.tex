...
injected key at $5.0\times$ the rate of rank-matched random edits
...
(c)~transitions via a pivot-like passage. H4 belongs to Phase~C and is \textbf{not
tested in this paper}: the pre-registration assigned Phase~A+B to the conference version
and Phase~C to the journal version, so H4's absence is a scope decision made in advance,
not a result withheld or a casualty of space. It has not been run.
\item[\textbf{H5}] (\emph{localization}) The layers at which an edit is causally
effective are confined to a limited band.
\end{itemize}

\noindent One deviation must be flagged here rather than in a footnote, because H3's
wording is what it violates. Our K1 control is matched in \emph{rank} but not in
\emph{norm}: the implementation draws a random orthonormal basis of the same shape and
never rescales the injected component. H3 as pre-registered asks for both. We report the
measured magnitudes instead of claiming a match: at the tested condition the edit applies
$\lVert\Delta\rVert = 32.5$ against K1's $28.7$ ($\lVert h\rVert \approx 100$), a ratio
of $1.13$---the two land close because both are rank-$24$ projections of comparably
scaled vectors, so the effect reported below is not an artifact of edit magnitude. But it
is a deviation from the frozen text, and it is logged as one.
...
step, whereas the \emph{probe} is read only at the positions where the model is about to
choose a pitch. We write to the whole stream and read from
...
$8/24$). Controls: \textbf{K1} rank-matched random subspace; \textbf{K2} sham
...
Othello-GPT's near-complete editability that we attribute to music's continual
