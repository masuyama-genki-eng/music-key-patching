% TonalWM — ICASSP 2027 submission
% ==========================================================================
...
without breaking everything else. Finally, a matched control settles what generalizes:
on real Bach chorales our synthetic-trained model does \emph{not} beat the pitch
surface, while a public model of \emph{identical architecture} trained on real music
does (margin $+0.198$, CI excluding zero)---with the training data as the single free
variable. Editing a music Transformer's internal key state changes what key it composes
in: tonality is tracked as a causal latent variable, not just surface statistics.
\end{abstract}
...
$85$M model reaches macro-$F_1$ $0.523$ against a selectivity floor of $0.037$---the key
subspace is real and transfers, it is not an artifact of our grammar---but the best input
baseline (KS, $W{=}64$) reaches $0.447$, and the corrected margin, $+0.034$ (CI
...
same decision rule; the two C3 baselines land at $0.447$ and $0.446$, confirming the
setups are matched. It reaches macro-$F_1$ $0.686$ (L$10$) with a corrected margin of
$+0.198$ (CI $[0.133,\,0.266]$): \textbf{DR-H1 supported}. With architecture, corpus,
...
Two cautions. The probe must be read where the model is about to \emph{choose} a pitch
...
depth and the key appears to vanish. Where one probes is not a free parameter. And this
is a readability result: we have not intervened on the public model, so it inherits none
of our causal claims.
...
validity:} the causal results are for our own small, synthetic-trained models;
Sec.~\ref{sec:wild} shows the \emph{readout} generalizes to a public real-trained model,
but intervening on such a model is future work. Whether edited continuations modulate
