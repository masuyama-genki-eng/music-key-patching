...
without breaking everything else. Editing a music Transformer's internal key
state changes what key it composes in: tonality is tracked as a causal latent
variable, not just surface statistics.
\end{abstract}
...
\textbf{The advantage over the surface does not survive distribution shift.} We probed
the same D-SYN-trained model on $300$ Bach chorales with human Roman-numeral analyses
supplying a \emph{local} key label at every token. The key subspace transfers: the
probe reaches macro-$F_1$ $0.41$ against a selectivity floor of $0.035$
($11\times$ chance), so the representation is not an artifact of our grammar. But it
does \emph{not} beat the input surface on real music: the best C3 baseline (KS,
$W{=}64$) reaches $0.43$, and the corrected margin is $-0.075$ (BCa $95\%$ CI
$[-0.151,-0.023]$)---negative, and excluding zero. (Scoring the same probe against a
piece-level \emph{global} key, which penalizes a local-key readout at every modulation,
is even harsher, $-0.147$; the fairer local test halves the deficit but does not flip
its sign.) H1's claim is therefore bounded more tightly than the D-SYN result alone
suggests: the internal state is more informative than the pitch surface \emph{where the
model is competent}, and a hand-designed estimator overtakes it once the input is out of
distribution. Whether a model \emph{trained} on real music recovers the advantage is the
question a public symbolic checkpoint would answer, and is the most valuable experiment
left open.

Four further observations bound our claims. \emph{V-DAS below V-PROBE:} the optimized
...
validity:} M-CTRL is small and synthetic-trained, so the claims concern emergence under
this condition, not commercial-scale models. Whether edited continuations modulate
