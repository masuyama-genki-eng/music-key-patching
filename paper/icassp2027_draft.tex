...
itself. A capacity sweep sharpens the claim: next-token accuracy is flat from
1.6M to 85M parameters, yet below ${\sim}6$M the key state does not exist ---
behaviorally equivalent models can be pure surface statistics, and the
pre-registered test tells them apart. Editing a music Transformer's internal
key state changes what key it composes in: tonality is tracked as a causal
latent variable, not just surface statistics.
\end{abstract}
...
\caption{The claim in one example (real model output, deterministic seed). The
...
onward. Nothing in the input changes; the continuation's estimated key follows
the injected internal state.}
\label{fig:framework}
\end{figure}
...
\caption{Readability is diffuse; causal efficacy is sharp. (a)~Linear-probe
macro-$F_1$ per layer for all six M-CTRL models (thin lines; R-Aug seed 0
highlighted) against the best input baseline: the key is legible across a broad
plateau (L2--L6). (b)~Causal effect [s0]: guarded strict $\tkr$ of the V-PROBE
edit vs.\ the K1 matched random control (bands: prompt-level bootstrap 95\% CI).
Only a narrow band around L4 actually moves the output.}
\label{fig:layers}
\end{figure}

\section{Results}
Unless noted, intervention numbers are from the R-Aug seed-0 sweep [s0]; probing
results hold for all six M-CTRL models, and the seed-1 sweep replicates every
Phase-B verdict (Sec.~\ref{sec:repl}).

\subsection{H1 --- the key is decodable beyond the input surface}
\begin{table}[t]
\centering\small
\caption{Key decoding at L4 (R-Aug seed 0; the layer the edit acts on --- probe
$F_1$ itself is flat across L2--L6), 24-class macro-$F_1$ on held-out sequences.
DR-H1 supported in 6/6 models.}
\label{tab:probe}
\begin{tabular}{@{}lc@{}}
\toprule
...
(Fig.~\ref{fig:equi}: $\varepsilon_{\mathrm{cyc}}\approx 0.98$ against
$\approx1.41$ for random orthogonal maps), and --- our pre-registered negative
result --- transposition augmentation does not improve it (3 seeds per regime:
R-Aug $0.983$ vs.\ R-NoAug $0.981$, one-sided $p=0.83$): DR-H2b is \emph{not
supported}. The

\begin{figure}[t]
\centering
\includegraphics[width=0.95\columnwidth]{fig_equivariance.pdf}
\caption{Cyclicity error of the transposition maps per layer (bars: mean of 3
seeds per regime; dots: individual seeds; dashed: random-orthogonal
reference). Transposition augmentation leaves the (non-cyclic) structure
unchanged --- the pre-registered DR-H2b is not supported.}
\label{fig:equi}
\end{figure}
...
\end{tabular}
\end{table}
Table~\ref{tab:tkr} and Fig.~\ref{fig:bars}: at the best in-budget condition
(V-PROBE, L4), $\tkr(\text{edit})-\tkr(\text{K1})>0$ holds for \textbf{12/12} targets
...
\caption{Guarded strict $\tkr$ across conditions [s0] (dotted line: chance
$1/12$): the probe-subspace edit at its best layer moves the composed key at
$5.6\times$ the matched random control and reaches $58\%$ of the
transposed-prompt behavioral reference.}
\label{fig:bars}
\end{figure}
\begin{figure}[t]
\centering
...
$0.03, 0.04, 0.19, 0.26, \mathbf{0.30}, 0.26, 0.23, 0.23$ for L0--L7;
best-vs-median bootstrap CI $[0.038, 0.109]$ excludes zero (DR-H5 supported).
The two profiles in Fig.~\ref{fig:layers} are not the same shape, and the
difference is informative: \emph{readability is diffuse where efficacy is
sharp}. The probe saturates across L2--L6 (macro-$F_1$ $0.920$--$0.929$, all
within $0.01$ of its maximum), so the key is legible from most of the network;
yet a single-layer edit only moves the output near L4, and barely at all at
L0--L1. Legibility of a state is thus a weak guide to where it is used --- a
caution for interpretability work that localizes a computation by probing
accuracy alone.

\subsection{Replication and capacity}
\label{sec:repl}
\textbf{Second seed.} The independently trained R-Aug seed-1 model reproduces
every Phase-B verdict: DR-H3 supported with V-PROBE significant in 12/12
targets ($\tkr$ $0.23$--$0.41$ vs.\ K1 $0.02$--$0.10$), and DR-H5 supported
(CI $[0.122, 0.181]$). Its single-peaked layer profile peaks at L2 rather than
L4, so the \emph{localization} replicates while its exact depth does not; we
report both profiles rather than a pooled one.

\begin{figure}[t]
\centering
\includegraphics[width=0.95\columnwidth]{fig_emergence.pdf}
\caption{Behavioral equivalence, representational dissociation. Across a
50$\times$ capacity range next-token accuracy is flat, but the key readout is
not: below ${\sim}6$M parameters the probe cannot beat the input baseline and
the pre-registered DR-H1 margin is negative. A behaviorally equivalent model
can lack the world model entirely. Dots: seeds.}
\label{fig:emergence}
\end{figure}

\textbf{Capacity (Fig.~\ref{fig:emergence}).} Training the same data and
protocol at 1.6M (L2, $d$=128), 6M (L4, $d$=256), 25M (main line) and 85M
(L12, $d$=768) parameters, next-token top-1 is \emph{flat} across a 50$\times$
capacity range ($0.868$--$0.879$), yet the internal key representation is not:
at 1.6M the probe (peak $F_1$ $0.63$--$0.66$) falls \emph{below} the input
baseline (corrected margin $-0.21$/$-0.17$; DR-H1 \emph{not supported} in
either seed), while from 6M upward it clears the baseline and saturates by 25M.
A model that is behaviorally equivalent on next-token prediction can therefore
lack the world model entirely --- it is the surface-statistics account made
flesh, and our probe correctly refuses to credit it with a key state. Whether
the causal effect emerges at the same threshold: \TBD{size-model intervention
sweeps running}. (The sweep varies depth and width together; disentangling them
...
are about emergence under this training condition, and the capacity sweep
varies depth and width together. Remaining robustness additions: probe
validation on real annotated music and a probe of a public large symbolic
checkpoint. The KS estimator confuses fifths-neighbors, which strict/tolerant
reporting brackets. Whether edited continuations \emph{modulate musically}
(pivot-like transitions vs.\ jumps) is deferred to the journal version.
