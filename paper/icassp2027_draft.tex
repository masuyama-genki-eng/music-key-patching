...
trained models, with the margin widening exactly where surface statistics are
least informative. (ii)~\emph{Geometry}: the twelve tonic directions
...
(R-NoAug) uniform 12-fold transposition augmentation --- $\times$ 2 seeds
(third seed: \TBD{training queued}). M-REF shares the architecture but a
...
results hold for all four M-CTRL models. Seed-1 sweep: \TBD{running}.
...
macro-$F_1$ on held-out sequences. DR-H1 supported in 4/4 models.}
\label{tab:probe}
\begin{tabular}{@{}lc@{}}
\toprule
...
\midrule
corrected margin (probe $-$ C1b $-$ C3), BCa 95\% CI &
$+0.105\;[0.091, 0.114]$\\
\bottomrule
\end{tabular}
\end{table}
Table~\ref{tab:probe}: the corrected probe--baseline margin excludes zero in
layers 1--7 of every model (peak $+0.105$ at L4), and only fails at L0
($-0.20$) --- the key is \emph{computed}, not transcribed from the input. In
the top ambiguity quartile (KS posterior entropy), probe accuracy is
$0.936$ vs.\ $0.795$ for the best baseline; the internal state is most
valuable exactly where the surface is least informative.
...
directions order themselves as C--G--D--A--E--B--F$\sharp$--\ldots{}: perfect
circle-of-fifths neighbor agreement (1.00) vs.\ none for the chromatic
ordering (0.00), at every probed layer (exploratory, no decision rule).
...
augmentation does not improve it (R-Aug $0.979$ vs.\ R-NoAug $0.982$,
one-sided $p=0.264$, $r=-0.19$): DR-H2b is \emph{not supported}. The
...
editing. Success decreases gently with target distance on the circle of
fifths ($0.395$ at distance 1 to $0.280$ at the tritone). The edit reaches
