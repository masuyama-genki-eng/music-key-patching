...
### F3 seed 1 と6モデルの held-out 再現

**状態**: 完了。seed 1 だけでなく，最終的には6モデルすべてに同じ手順を適用した。
各モデルで search prompt 上の層走査を行い，guarded edit margin 最大の層を選んでから
final test を行った。`R-Aug_s1` は層2で，長調 0.289・短調 0.392。6モデル全体では
長調 0.231--0.355，短調 0.280--0.495 で，すべてのモデルが対応する対照を上回る。
ただし本文の主モデル `R-Aug_s0` は両旋法で最大であり，この点は本文・補足で明記する。

**参照**: `results/seed_robustness.json`，各モデルの
`results/sweep/<model>/layer_selection_margin.json`。
...
現在の貼り付け版は `latexmk` で**5ページ**としてコンパイルされる。今後の追記で再び
紙幅を超える可能性があるため，図表番号・本文参照・補足参照を更新したあとに必ず
PDFでページ数を確認する。
